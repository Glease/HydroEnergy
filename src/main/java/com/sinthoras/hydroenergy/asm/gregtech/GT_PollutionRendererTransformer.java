...
public class GT_PollutionRendererTransformer implements IClassTransformer {
    ...
    public static byte[] transform(byte[] basicClass, boolean isObfuscated) {
        ...
        if(isApiUsed) {
            HEPlugin.info("GregTech5U(" + fullClassName + ") is using HydroEnergy API. No injection necessary.");
            return basicClass;
        }
        HEPlugin.info("GregTech5U(" + fullClassName + ") does not use HydroEnergy API. Fallback to injection.");
        ...
        if(targetMethod == null) {
            HEPlugin.warn("Could not find method " + MARKER_method + ":" + MARKER_method_DESC + " in " + fullClassName + ". You might experience visual bugs.");
            return basicClass;
        }
        ...
        if(instructions.size() != 2) {
            HEPlugin.warn("Could not find instruction " + MARKER_instruction_OWNER + "." + MARKER_instruction + ":" + MARKER_instruction_DESC + " twice in said method. You might experience visual bugs.");
            return basicClass;
        }
        ...
        HEPlugin.info("Fixed mod-interop with GregTech5U.");
