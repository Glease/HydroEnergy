...
public class WorldTransformer {
    ...
    private static byte[] transformHandleMaterialAcceleration(byte[] basicClass, boolean isObfuscated) {
        ...
        if(targetMethod == null) {
            HEPlugin.error("Could not find injection target method in World. HydroEnergy will not work.");
            return basicClass;
        }
        ...
        if(instructions.size() != 1) {
            HEPlugin.error("Could not find injection target instruction in World. HydroEnergy will not work.");
            return basicClass;
        }
